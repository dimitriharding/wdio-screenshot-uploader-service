...
class Uploaders {
	...
	constructor(screenshotUploaderOptions, config, capabilities) {
		...
		if (this._storagesInfo[AZURE_BLOB]) {
			this._blobService = azure.createBlobService(options.storagesInfo[AZURE_BLOB].accountName, options.storagesInfo[AZURE_BLOB].accountKey);
			return new Promise((resolve, reject) => {
				this._blobService.createContainerIfNotExists(options.storagesInfo[AZURE_BLOB].bucket || 'wdio-screenshots', { publicAccessLevel: 'blob' }, err => {
					if (err) {
						reject(err);
					} else {
	 ...
	 * @return {Object}
	 */
	[AZURE_BLOB](screenshotData) {
		const {filePath, name} = screenshotData;
		return new Promise((resolve, reject) => {
			this._blobService.createBlockBlobFromLocalFile(options.storagesInfo[AZURE_BLOB].bucket || 'wdio-screenshots', name, filePath, err => {
				if (err) {
					this.logUploadFailed();
					reject(err);
				} else {
					let data = {uploader: AZURE_BLOB};
					resolve(data);
					this.logUploadSuccess();
				}
			});
